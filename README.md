...
1. Fork the repository and make changes on a branch.
2. Test your changes locally (especially install scripts).
3. Open a pull request with a clear description of the change and any manual steps required to test.

When adding features, prefer small, focused changes and include clear README updates for any new behavior or configuration options.

---

## License

This repository contains a mixture of configuration and scripts. Check top-level files and individual sub-folders for license notices (for example, `configs/nvim/LICENSE` exists for the Neovim portion). If no explicit license is provided for the repository as a whole, treat the contents as "no license" and contact the repository owner for permission before reuse.

---

If you want, I can:
- Generate a quick checklist of files to review before running the installer.
- Add example symlink commands to deploy a specific config (e.g., Neovim) into `~/.config`.
- Create a small script to preview what `install.sh` will do without making changes.
